...
- **Was ist was:** Ein typischer STT-Fehler ist ein Wort, das im Kontext offensichtlich etwas anderes meint (Bart/Bad, hirichlich/richtig, Loket, Schwimmbäck/Schwimmbecken, um Lätt/live, nicht ziemlich wach/nicht besonders). Ein Stil-Merkmal ist eine Wortwahl, die Hanno tatsächlich benutzt („Kutenberg" statt „Kutná Hora", „Modell" statt „Modul" wenn er es so meint, mündliche Wendungen). Ein Brabbel-Artefakt ist Sprache, die beim Diktieren entsteht (Abbrüche, Füllwörter, Doppelungen) — das wird geglättet, ohne den Sinn zu verändern. Im Zweifel nicht inhaltlich anfassen — Hanno fragen oder stehen lassen.
